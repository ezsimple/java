...
public class Function {
	...
	public static JSONObject makeJsp(String name, String path, String newSrc){
		JSONObject result = new JSONObject();
		File f;
		File df;
		...
		String readSrcBef;
		String readSrcAft;
		boolean noChangeSrc = true;
		long oshash;
		long nshash;
		newSrc = "%>\n" + newSrc + "\n<%//";
		nshash = newSrc.replaceAll(" ", "").replaceAll("\n", "").replaceAll("\r", "").replaceAll("\t", "").hashCode();
		String fullPath = ProcessorServiceFactory.getQueryFullPath() + "/../jsp/ui/" + path;
		
		df = new File(fullPath);
		...
		try {
			f = new File(fullPath, name+".jsp");
			if(f.exists()){
				readSrc = FileUtils.readFileToString(f, "utf-8");
			}else{
				readSrc = FileUtils.readFileToString(new File(ProcessorServiceFactory.getQueryFullPath()+"/../jsp/admin/src", "tpl.jsp"), "utf-8");
			}
			
			oldSrc = StringUtils.substringBetween(readSrc, "[[START_AUTO_GENERATED_SRC]]", "[[END_AUTO_GENERATED_SRC]]");
			
			oshash = oldSrc.replaceAll(" ", "").replaceAll("\n", "").replaceAll("\r", "").replaceAll("\t", "").hashCode();
			
			readSrcBef = StringUtils.substringBefore(readSrc, "[[START_AUTO_GENERATED_SRC]]");
			String oldHashCode = StringUtils.substringBetween(readSrc, "[[END_AUTO_GENERATED_SRC]]", "[[HASHCODE]]");
			result.put("message", "소스가 생성되었습니다.");
			
			if(StringUtils.isNotEmpty(oldHashCode) && !Long.toString(oshash).equals(oldHashCode)){
				noChangeSrc = true;
				df =  new File(fullPath, name+"."+dateFormater.format(new Date()));
				FileUtils.copyFile(f, df);
				result.put("message", "소스가 변경되어 백업하고 새로 생성 하였습니다.");
			}
			readSrcAft = StringUtils.substringAfterLast(readSrc, "[[HASHCODE]]");
			src.append(readSrcBef).append("[[START_AUTO_GENERATED_SRC]]").append(newSrc).append("[[END_AUTO_GENERATED_SRC]]").append(nshash).append("[[HASHCODE]]").append(readSrcAft);
			
			FileUtils.writeStringToFile(f, src.toString(), "utf-8");
		} catch (Exception e) {
			noChangeSrc = false;
			result.put("message", e.toString());
		}
		result.put("success", noChangeSrc);
		return result;
	}
}
