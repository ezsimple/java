	...
	$(function() {
		...
		initPortletEvent();
		//라디오버튼 재 선택시 해제 처리
		$(document).on('mousedown', '[type=radio]', function(event){
			var target = $(event.target);
			
			if(target.context.nodeName=='LABEL'){
	...
	function resizeWindow(){
		$( window ).resize(function(e,e1) {
			...
			if($['reDrawChartByReSize']){
				setTimeout( $['reDrawChartByReSize'], 100);
			}
		}).resize();
	}
	...
	function initLoadingPage(){
		try {
			...
			//셀렉트 박스초기화
			var selects = $( ".select" ).selectmenu();
			
			for(var i=0;i<selects.length; i++){
				$(selects[i]).selectmenu( "menuWidget" ).addClass( "overflow" );	
			}
			...
			for(var i=0; i<btns.length;i++){
				var btn = $(btns[i]);
				btn.button({
					text:!(btn.attr('text')==='false'),
					icons: {
						primary: btn.attr('icon_primary'),
						secondary: btn.attr('icon_secondary')
					}
				});
			}
