...
function saveUi(isMake){
	...
	$('#ui_set').load('../admin-src/src_save.sh', data, function(){
		if(isMake){
			var url = 'src_make.sh?ui_id=' + $('#ui_id').val();
...
function makeSrc(){
	if(!confirm("저장하고 소스를 생성할까요?")){
		return;
	}
	saveUi(true);
}
function viewUiList(){
	$('#ui_list').load('../piece/-ui_list-.sh', function(){
		showMenu();
	});
}
/* 	
...
<body >
	<div class="menu" type="user" style="width: 320px;">
		...
		<div id="ui_list" class="auto_height" style="height:700px; overflow-y: auto;"></div> 
	</div>


	<form id="main_form" action="aa" method="post">
 		<input type="hidden" id="ui_design" name="ui_design" value="">
		<div id="defaultData"  class="ui-widget-header" style="float: left;padding:1px;">
			<div class=" f_l p_1  " >
				UI ID : <input type="text" id="ui_id" name="ui_id" value="${param.ui_id }">
			</div>
			<div class=" f_l p_1  " >
				&nbsp;쿼리경로 : <tag:select_query_name name="queryPath" selected="${req.queryPath }"/>
			</div>
<!-- 			<div class=" f_l p_1  " >
				&nbsp;기본값 : <input type="text" id="defaultValue" name="defaultValue" style="width: 200px;" value="rows:10,_start:1,notice_id:72">
			</div>
 -->		</div>
		
		<div class="ui-state-default ui-corner-all  btn_left f_l" onclick="loadPage()">읽기</div>
		<div class="ui-state-default ui-corner-all btn_left f_l" onclick="saveUi()">저장</div>
		<div class="ui-state-default ui-corner-all btn_left f_l" onclick="makeSrc()" >소스생성</div>
		<div class="ui-state-default ui-corner-all btn_left f_l" onclick="runDefaultPage()" >실행</div>
		...
		<div id="source" style="clear: both;border: 1px solid #c5dbec;padding: 5px; min-height: 100px;">		
