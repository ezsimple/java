							...
							<ul class="media-list">
								<li class="media">
									...
									<div class="media-body">
										<h4 class="media-heading btn btn-danger btn-lg">DR 이벤트 발령</h4>
										<h3 style="color:red;">이벤트 일시 : <span id="event_start_date"></span></h3>
	...
	<div class="row">
		...
		<%--<div class="col-xs-12 col-sm-12">--%>
			...
			<%--<form id="form_" action="" method="post" enctype="multipart/form-data">--%>
				<%--<input type="hidden" name="ui_id" value="../-at-lo-pg/-home/.sh">--%>
			<%--</form>--%>
			<%--<a href="javascript:reDashBoard();">가자</a>--%>
		<%--</div>--%>
    ...
    $(document).ready(function() {
        ...
        var pagefunction = function (){
            ...
            function draw_g_1() {
                ...
                g3 = new Dygraph(document.getElementById("noroll"), cblData, {
                    showRoller : true,
                    customBars : false,
                    ylabel : '전력사용량 (kWh)',
                    legend : 'always',
        ...
        function loadEvent(){
            $.getJSON( "../ai/aptEvent.curentEvent", function( data ) {
                ...
                if(data.row.event_status){
                    ...
                    $('#event_status').text(data.row.event_status.toLowerCase()=='active' ? '진행중' : '대기중');
        ...
        // var tId = -1;
        var reDashBoard = function() {
            <%
                // var url = $('#mDashBoard').attr('href');
                // window.location.hash = url;
                // window.location.reload();
            %>
            if (_tId != -1) { clearTimeout(_tId); _tId = -1; }
            $('#widget-grid').load('../-at-lo-pg/-home.sh')
        }
        var tv = (5* 60 * 1000) <%//한전정보 읽을시 주의 : 1분 간격은 socketTimeOut 발생 %>
        _tId = setTimeout (reDashBoard, tv);
    });
</script>
<style>
/*
...
*/
#noroll, #roll14 {width:100%; height:300px;}
