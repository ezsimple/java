...
<c:if test="${empty(_code) }">
	<sp:sp var="SELECT_RESULT" queryPath="ui" action="codeList" processorList="mybatis" exception="false"/>
	<tag:list2group var="_code" group_field="group_id" list="${rows }"/>
</c:if>
<select name="${name }"  valid="${valid }" style="${style }"  ${attr } class="${className }" >
