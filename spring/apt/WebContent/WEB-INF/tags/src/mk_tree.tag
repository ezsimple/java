...
<c:forEach var="info" items="${rows[0] }" >
	...
	<c:if test="${!empty(ui_field[link]) }">
		<c:set var="link_value">${ui_field[link_type] }('#tree_${'$'}{tree_id }', ${'<'}tag:el source="${fn:replace(ui_field[link], '$', _$) }" param="${'$'}{rows[0] }"/>)</c:set>
	</c:if>
</c:forEach>
<script type="text/javascript">
$(function(){

	var treeData = ${'$'}{sp:list2tree(rows, '${upperFld}', '${codeFld}', '${titleFld}', '${idFld}', '${rootId}') };
